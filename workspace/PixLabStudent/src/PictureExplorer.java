...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener {
	...
	public static void main(String args[]) {		
		Picture pix1 = new Picture("blue-mark.jpg");
		Picture pix2 = new Picture("water.jpg");
		Picture scaled = new Picture(pix1);
		Pixel[][] pixel1 = pix1.getPixels2D();
		Pixel[][] pixel2 = pix2.getPixels2D();
		Pixel[][] pixelScaled = scaled.getPixels2D();
		pix1.edgeDetection(32);
		pix1.explore();


		for (int i = 0; i < pixel1.length; i++) {
			int row = pixel2.length * i / pixel1.length;
			for (int j = 0; j < pixel1[i].length; j++) {
				int col = pixel2[row].length * j / pixel1[i].length;
				
				pixelScaled[i][j].setColor(pixel2[row][col].getColor());
			}
		}
		
		
		int deepFry = 0b10101010;
		double scale = 1;
		
		for (int i = 0; i < pixel1.length; i++) {
			for (int j = 0; j < pixel1[i].length; j++) {
				pixel1[i][j].setRed(pixel1[i][j].getRed() ^ pixelScaled[i][j].getRed());
				pixel1[i][j].setGreen(pixel1[i][j].getGreen() ^ pixelScaled[i][j].getGreen());
				pixel1[i][j].setBlue(pixel1[i][j].getBlue() ^ pixelScaled[i][j].getBlue());
			}
		}
		
		//pix.getPixels2D()[0][0].setColor(ColorChooser.pickAColor());
		pix1.explore();
	}
	/*
	public static void main(String args[]) {		
		Picture pix = new Picture("blue-mark.jpg");
		Picture edge = new Picture(pix.getFileName());
		Picture and = new Picture(pix.getFileName());
		Picture or = new Picture(pix.getFileName());
		Picture xor = new Picture(pix.getFileName());
		//edge.copyPicture(pix);
		for (int i = 32; i >= 16; i -= 8) {
			edge.edgeDetection(i);
		}
		edge.explore();
		//edge.edgeDetection(-10);
		
		Pixel[][] pixPixels = pix.getPixels2D();
		Pixel[][] edgePixels = edge.getPixels2D();
		Pixel[][] andPixels = and.getPixels2D();
		Pixel[][] orPixels = or.getPixels2D();
		Pixel[][] xorPixels = xor.getPixels2D();
		pix.explore();
		
		int deepFry = 0b11110000;
		double scale = 1;
		
		for (int i = 0; i < pixPixels.length; i++) {
			float hue = (float) i / pixPixels.length;
			for (int j = 0; j < pixPixels[i].length; j++) {
				float sat = (float) j / pixPixels[i].length;
				Color c = Color.getHSBColor(hue, sat, 1f);
				edgePixels[i][j].setRed(edgePixels[i][j].getRed() ^ c.getRed());
				edgePixels[i][j].setGreen(edgePixels[i][j].getGreen() ^ c.getGreen());
				edgePixels[i][j].setBlue(edgePixels[i][j].getBlue() ^ c.getBlue());
				
				andPixels[i][j].setRed(pixPixels[i][j].getRed() & edgePixels[i][j].getRed());
				andPixels[i][j].setGreen(pixPixels[i][j].getGreen() & edgePixels[i][j].getGreen());
				andPixels[i][j].setBlue(pixPixels[i][j].getBlue() & edgePixels[i][j].getBlue());
				
				orPixels[i][j].setRed(pixPixels[i][j].getRed() | edgePixels[i][j].getRed());
				orPixels[i][j].setGreen(pixPixels[i][j].getGreen() | edgePixels[i][j].getGreen());
				orPixels[i][j].setBlue(pixPixels[i][j].getBlue() | edgePixels[i][j].getBlue());
				
				xorPixels[i][j].setRed(pixPixels[i][j].getRed() ^ edgePixels[i][j].getRed());
				xorPixels[i][j].setGreen(pixPixels[i][j].getGreen() ^ edgePixels[i][j].getGreen());
				xorPixels[i][j].setBlue(pixPixels[i][j].getBlue() ^ edgePixels[i][j].getBlue());
			}
		}
